...
object ItemGen {
    ...
    val itemOverrides =
        mapOf(
            // Warglaives
            32837 to mapOf("className" to "WarglaiveOfAzzinothMH", "name" to "Warglaive of Azzinoth (MH)"),
            32838 to mapOf("className" to "WarglaiveOfAzzinothOH", "name" to "Warglaive of Azzinoth (OH)"),
    ...
    fun generate() {
        ...
        val protoItems =
            itemsData
                .map {
                    ...
                    item.itemClass = Constants.ItemClass.values().find { it2 -> it2.ordinal == it["class"] as Int? }
                    item.itemSubclass =
                        Constants.ItemClass.subclasses(item.itemClass).find { it2 ->
                    ...
                    val phase =
                        phaseOverride ?: itemPhases[item.id.toString()] ?: itemPhasesApprox[item.id.toString()] ?: 0
                    item.phase = phase
        ...
        protoItems.forEach { itemPair -> writeItemClassFile(itemPair.first, itemPair.second, itemBuffsData) }
    ...
    private fun writeItemIndexFile(items: List<Item>) {
        ...
        val itemsListBlock =
            items
                .map { CodeBlock.of("{ %L() }", safeItemName(it)) }
                .joinToCode(separator = ",\n", prefix = "arrayOf(\n", suffix = "\n)\n")

        objBuilder.addProperty(
            PropertySpec.builder(
                    "items",
                    ARRAY.parameterizedBy(LambdaTypeName.get(returnType = ClassName("data.model", "Item"))),
                )
                .initializer("%L", itemsListBlock)
                .build()
        )

        FileSpec.builder(pkg, "ItemIndex").addType(objBuilder.build()).build().writeTo(File(CodeGen.outPath))
    }
    ...
    private fun shouldOverwrite(name: String): Boolean {
        return try {
            ...
            return klass.getDeclaredMethod("isAutoGenerated").invoke(klass.getDeclaredConstructor().newInstance())
                as Boolean
    ...
    private fun renderStats(item: Item, itemData: Map<String, Any?>): CodeBlock {
        ...
        val block =
            fields
                .mapNotNull {
                    val method =
                        Class.forName("character.Stats")
                            .getDeclaredMethod("get${it.name.replaceFirstChar { c -> c.uppercase() }}")
        ...
        return CodeBlock.builder().add("%T", Stats::class).add(block).build()
    }

    private fun renderSockets(item: Item, itemData: Map<String, Any?>): CodeBlock {
        return if (item.sockets.isNotEmpty()) {
            item.sockets
                .map { CodeBlock.of("%1T(%2T.%3L)", Socket::class, Color::class, it.color) }
                .joinToCode(separator = ",\n", prefix = "arrayOf(\n", suffix = "\n)")
        } else {
            CodeBlock.builder().add("%L", "arrayOf()").build()
        }
    }
    ...
    private fun renderBuffs(
        item: Item,
        itemData: Map<String, Any?>,
        itemBuffsData: List<Map<String, Any?>>,
    ): CodeBlock {
        val buffIds =
            (1..5).mapNotNull { i ->
                ...
                if (spellId == 0) {
                    ...
                    val itemBuff =
                        itemBuffsData.find { it["Id"] as Int == spellId }
        ...
        return if (buffIds.isNotEmpty()) {
            buffIds
                .map { CodeBlock.of("%T.byIdOrName(%L, %S, %L)", Buffs::class, it.first, it.second, "this") }
                .joinToCode(separator = ",\n", prefix = "listOfNotNull(\n", suffix = "\n)")
        } else {
            CodeBlock.builder().add("%L", "listOf()").build()
        }
    }
    ...
    private fun renderAllowableClasses(item: Item, itemData: Map<String, Any?>): CodeBlock {
        ...
        if (classMask != null && classMask != -1) {
            val classes = Constants.AllowableClass.values().filter { it() and classMask != 0 }

            return classes
                .map { CodeBlock.of("%T.%L", Constants.AllowableClass::class, it.name) }
                .joinToCode(separator = ",\n", prefix = "arrayOf(\n", suffix = "\n)")
        } else {
                ...
                FileSpec.builder(pkg, className)
                    .addType(
                        TypeSpec.classBuilder(className)
                            ...
                            .addAnnotation(AnnotationSpec.builder(ClassName.bestGuess("kotlin.js.JsExport")).build())
                            .addProperty(
                            ...
                            .addProperty(
                                PropertySpec.builder(
                                        "itemSubclass",
                                        Constants.ItemSubclass::class.asTypeName().copy(true),
                                    )
                                    .addModifiers(KModifier.OVERRIDE)
                                    .mutable(true)
                            ...
                            .addProperty(
                                PropertySpec.builder(
                                        ...
                                        Array::class.asTypeName()
                                            .parameterizedBy(Constants.AllowableClass::class.asTypeName())
                                            .copy(true),
                                    )
                                    .addModifiers(KModifier.OVERRIDE)
                                    .mutable(true)
                            ...
                            .addProperty(
                                PropertySpec.builder(
                                        "sockets",
                                        ARRAY.parameterizedBy(ClassName("data.model", "Socket")),
                                    )
                                    .addModifiers(KModifier.OVERRIDE)
                                    .mutable(true)
