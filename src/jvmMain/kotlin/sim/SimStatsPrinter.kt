...
object SimStatsPrinter {
    ...
    fun precombatStats(iteration: SimIteration) {
        ...
        iteration.subject.rotation.rules
            .filter { it.phase == Rotation.Phase.RAID_OR_PARTY }
            .forEach { println(" - ${it.ability.name}") }
        println()
    }
    ...
    fun printDps(subjectDps: DpsBreakdown, subjectPetDps: DpsBreakdown?) {
        ...
        println(
            "AVERAGE DPS: ${df.format(subjectDps.mean)}" +
                if (subjectPetDps != null) {
                    " - PET: ${df.format(subjectPetDps.mean)} (${df.format(petMeanPct)}%) - TOTAL: ${df.format(subjectDps.mean + subjectPetDps.mean)}"
                } else ""
        )
        println(
            "MEDIAN DPS: ${df.format(subjectDps.median)}" +
                if (subjectPetDps != null) {
                    " - PET: ${df.format(subjectPetDps.median)} (${df.format(petMedianPct)}%) - TOTAL: ${df.format(subjectDps.median + subjectPetDps.median)}"
                } else ""
        )
        println(
            "STDDEV DPS: ${df.format(subjectDps.sd)}" +
                if (subjectPetDps != null) {
                    " - PET: ${df.format(subjectPetDps.sd)}"
                } else ""
        )
    }
        ...
        println(
            "PLAYER STATS\n" +
                table {
                        ...
                        row(
                            "Intellect:",
                            iteration.subject.intellect(),
                            "Ranged Crit:",
                            iteration.subject.rangedCritPct(),
                        )
                        row(
                            "Stamina:",
                            iteration.subject.stamina(),
                            "Phys. Haste:",
                            iteration.subject.physicalHasteMultiplier() - 1.0,
                        )
                        ...
                        row(
                            "Attack Power",
                            iteration.subject.attackPower(),
                            "Spell Haste:",
                            iteration.subject.spellHasteMultiplier() - 1.0,
                        )
                        row(
                            "R. Attack Power",
                            iteration.subject.rangedAttackPower(),
                            "Expertise:",
                            iteration.subject.expertisePct(),
                        )
