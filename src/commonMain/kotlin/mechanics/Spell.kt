...
object Spell {
    ...
    val baseMissChance = mapOf(0 to 0.04, 1 to 0.05, 2 to 0.06, 3 to 0.17)

    // https://www.youtube.com/watch?v=7DXvTVfDN18
    // Note that this cannot be reduced by spell pen or other mechanics
    val baseSpellResistance = mapOf(0 to 0, 1 to 5, 2 to 10, 3 to 15)

    // Partial resists are calculated in terms of percentage of cap
    // The function is piecewise in thirds - each bracket scales linearly (but differently) between
    // 0%, 33.3%, 66.6% and 100%
    ...
    fun spellResistReduction(sp: SimParticipant, school: Constants.DamageType, isBinary: Boolean): Double {
        ...
        // Resistance brackets are a piecewise function which, when weighted, average to the target
        // average resistance
        val resistCap = 5 * sp.character.level
        val totalResistance =
            ((targetResistance - sp.stats.spellPen).coerceAtLeast(0) + baseResistance).coerceAtMost(resistCap)
        ...
        val chanceByQuartileBase =
            resistWeights.map {
                // Then, find where we are on each line to get a percentage chance of this being the
                // outcome
        ...
        val chanceByQuartile =
            chanceByQuartileBase.mapIndexed { idx, it ->
    ...
    fun baseDamageRollSingle(
        sp: SimParticipant,
        baseDmg: Double,
        school: Constants.DamageType,
        spellDamageCoeff: Double = 1.0,
        bonusSpellDamage: Int = 0,
        bonusSpellDamageMultiplier: Double = 1.0,
    ): Double {
        // Add school damage
    ...
    fun partialResistRoll(
        sp: SimParticipant,
        damageEvent: Pair<Double, EventResult>,
        school: Constants.DamageType,
    ): Pair<Double, EventResult> {
        val resistReduction = spellResistReduction(sp, school, false)
        val resistEventResult =
            when (resistReduction) {
                0.0 -> damageEvent.second
                else ->
                    when (damageEvent.second) {
