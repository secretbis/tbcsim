...
class AutoShot : AutoAttackBase() {
    ...
    override fun cast(sp: SimParticipant) {
        ...
        val ammoBonusAp =
            if (sp.buffs[NoAmmo.name] == null) {
                General.dpsToAp(sp.character.gear.ammo.maxDmg)
            } else 0
        ...
        val triggerTypes =
            when (result.second) {
                EventResult.HIT ->
                    listOf(Proc.Trigger.RANGED_AUTO_HIT, Proc.Trigger.RANGED_WHITE_HIT, Proc.Trigger.PHYSICAL_DAMAGE)
                EventResult.CRIT ->
                    listOf(Proc.Trigger.RANGED_AUTO_CRIT, Proc.Trigger.RANGED_WHITE_CRIT, Proc.Trigger.PHYSICAL_DAMAGE)
