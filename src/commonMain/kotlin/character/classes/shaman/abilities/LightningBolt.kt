...
open class LightningBolt : Ability() {
    ...
    private fun actualCast(sp: SimParticipant, isLoProc: Boolean = false) {
        ...
        val t6Multiplier =
            if (t6Bonus) {
                SkyshatterRegalia.fourSetLBDamageMultiplier()
            } else 1.0
        ...
        val damageRoll =
            Spell.baseDamageRoll(
                ...
                school,
                spellPowerCoeff,
                totemSpellDmgBonus + totemAgSpellDmgBonus,
            ) * concussionMod * loMod * t6Multiplier
        ...
        val baseTriggerTypes =
            if (isLoProc) {
                listOf()
            } else listOf(Proc.Trigger.SHAMAN_CAST_LIGHTNING_BOLT)
        val triggerTypes =
            when (result.second) {
                EventResult.HIT -> listOf(Proc.Trigger.SPELL_HIT, Proc.Trigger.NATURE_DAMAGE)
                EventResult.CRIT ->
                    listOf(Proc.Trigger.SHAMAN_CRIT_LIGHTNING_BOLT, Proc.Trigger.SPELL_CRIT, Proc.Trigger.NATURE_DAMAGE)
