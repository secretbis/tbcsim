...
class Hemorrhage(owner: SimParticipant) : Debuff(owner) {
    ...
    // TODO: maybe reduce this to a number that represents something realistic with other raid
    // members
    // consuming charges,
    ...
    override fun modifyStats(sp: SimParticipant): Stats {
        return Stats(whiteDamageFlatModifier = physicalDmgIncrease, yellowDamageFlatModifier = physicalDmgIncrease)
    }
    ...
    fun makeProc(buff: Debuff): Proc {
        return object : Proc() {
            override val triggers: List<Trigger> = listOf(Proc.Trigger.PHYSICAL_DAMAGE)
