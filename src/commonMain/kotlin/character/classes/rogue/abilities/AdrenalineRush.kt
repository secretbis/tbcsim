...
class AdrenalineRush : Ability() {
    ...
    override fun available(sp: SimParticipant): Boolean {
        val ar =
            sp.character.klass.talents[character.classes.rogue.talents.AdrenalineRush.name]
                as character.classes.rogue.talents.AdrenalineRush?
        val available =
            if (ar != null) {
                ar.currentRank == ar.maxRank
            } else {
                false
            }
        if (!available) {
