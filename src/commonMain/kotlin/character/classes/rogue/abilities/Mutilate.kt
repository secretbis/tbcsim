...
open class Mutilate : Ability() {
    ...
    override fun available(sp: SimParticipant): Boolean {
        val mutilate =
            sp.character.klass.talents[character.classes.rogue.talents.Mutilate.name]
                as character.classes.rogue.talents.Mutilate?
        val available =
            if (mutilate != null) {
                mutilate.currentRank == mutilate.maxRank
            } else {
                false
            }
        if (!available) {
        ...
        val usesDaggers =
            if (sp.isDualWielding()) {
                (sp.character.gear.mainHand.itemSubclass == Constants.ItemSubclass.DAGGER) &&
                    (sp.character.gear.offHand.itemSubclass == Constants.ItemSubclass.DAGGER)
            } else {
                false
            }
        if (!usesDaggers) {
    ...
    fun castMainhand(sp: SimParticipant, bonusDamage: Double, poisonMultiplier: Double) {
        ...
        increasedDamagePercent +=
            if (slayers != null) {
                SlayersArmor.fourSetGeneratorDamageIncreasePercent()
            } else 0.0
        ...
        val result =
            Melee.attackRoll(
                ...
                item,
                isWhiteDmg = false,
                bonusCritChance = increasedCritChance,
                abilityAdditionalCritDamageMultiplier = critDmgMultiplier,
            )

        if (
            result.second != EventResult.MISS &&
                result.second != EventResult.DODGE &&
                result.second != EventResult.PARRY
        ) {
    ...
    override fun cast(sp: SimParticipant) {
        val poisonMultiplier =
            if (poisonsPresentOnTarget(sp)) {
                1.5
            } else {
                1.0
            }
