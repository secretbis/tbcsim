...
class Eviscerate : FinisherAbility() {
    ...
    fun damage(sp: SimParticipant, cps: Int, dmgMultiplier: Double): Pair<Double, Double> {
        ...
        return when (consumedComboPoints) {
            1 ->
                Pair(
                    60 * dmgMultiplier + 185 + modifier + (bonusDamagePerCP * 1),
                    180 * dmgMultiplier + 185 + modifier + (bonusDamagePerCP * 1),
                )
            2 ->
                Pair(
                    60 * dmgMultiplier + 370 + modifier + (bonusDamagePerCP * 2),
                    180 * dmgMultiplier + 370 + modifier + (bonusDamagePerCP * 2),
                )
            3 ->
                Pair(
                    60 * dmgMultiplier + 555 + modifier + (bonusDamagePerCP * 3),
                    180 * dmgMultiplier + 555 + modifier + (bonusDamagePerCP * 3),
                )
            4 ->
                Pair(
                    60 * dmgMultiplier + 740 + modifier + (bonusDamagePerCP * 4),
                    180 * dmgMultiplier + 740 + modifier + (bonusDamagePerCP * 4),
                )
            5 ->
                Pair(
                    60 * dmgMultiplier + 925 + modifier + (bonusDamagePerCP * 5),
                    180 * dmgMultiplier + 925 + modifier + (bonusDamagePerCP * 5),
                )
            else -> Pair(0.0, 0.0)
        }
    }
