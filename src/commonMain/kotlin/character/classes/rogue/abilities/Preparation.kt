...
class Preparation : Ability() {
    ...
    override fun available(sp: SimParticipant): Boolean {
        val prep =
            sp.character.klass.talents[character.classes.rogue.talents.Preparation.name]
                as character.classes.rogue.talents.Preparation?
        val available =
            if (prep != null) {
                prep.currentRank == prep.maxRank
            } else {
                false
            }
        if (!available) {
