...
class SliceAndDice(sp: SimParticipant, consumedComboPoints: Int) : Buff() {
    ...
    override fun mutexPriority(sp: SimParticipant): Map<Mutex, Int> {
        return mapOf(Mutex.BUFF_SLICE_AND_DICE to this.remainingDurationMs(sp))
    }
    ...
    fun getDurationForCombopoints(sp: SimParticipant, consumedComboPoints: Int): Int {
        ...
        val durationBonus =
            if (netherblade != null) {
                Netherblade.twoSetSnDDurationIncreaseMs()
            } else 0
    ...
    override fun modifyStats(sp: SimParticipant): Stats {
        ...
        val hasteBonus =
            if (slayers != null) {
                SlayersArmor.twoSetAdditionalSnDHaste()
            } else 0.0

        // TODO: might be multiplicative
        return Stats(physicalHasteMultiplier = 1.3 + hasteBonus)
    }
}
