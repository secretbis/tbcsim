...
class Stealth : Buff() {
    ...
    val stealthAbilities: Set<String> = setOf(Garrote.name, Ambush.name)
        ...
        return object : Proc() {
            override val triggers: List<Trigger> =
                listOf(
                    ...
                    // Trigger.SPELL_CAST  // could use this and just exclude things that don't
                    // break
                    // stealth, would have to explude all precombat stuff though
                )
            override val type: Type = Type.STATIC

            override fun shouldProc(sp: SimParticipant, items: List<Item>?, ability: Ability?, event: Event?): Boolean {
                // lets just assume the rotation makes sure no other abilities are cast when stealth
                // is on.
