...
class Ignite(currentRank: Int) : Talent(currentRank) {
    ...
    val buff =
        object : Buff() {
            ...
            val proc =
                object : Proc() {
                    override val triggers: List<Trigger> = listOf(Trigger.FIRE_DAMAGE_NON_PERIODIC)
                    override val type: Type = Type.STATIC

                    fun igniteDebuff(sp: SimParticipant): Debuff =
                        object : Debuff(sp) {
                            override val name: String = Companion.name
                            override val icon: String = "spell_fire_incinerate.jpg"
                            ...
                            override fun tick(sp: SimParticipant) {
                                ...
                                val igniteTickDamage =
                                    state.damageEvents.fold(0.0) { acc, evt -> acc + (evt.first * tickMultiplier) }
                                ...
                                state.damageEvents =
                                    state.damageEvents
                                        .filter { it.second > sp.sim.elapsedTimeMs - 4000 }
                                        .toMutableList()
                            }
                        }
