...
class ArcaneBlast : Ability() {
    ...
    override fun resourceCost(sp: SimParticipant): Double {
        ...
        val t5Mult =
            if (t5Buff != null) {
                TirisfalRegalia.twoSetArcaneBlastDamageAndCostMultiplier()
            } else 1.0

        // From testing, these stack based on the base mana cost
        return baseResourceCost +
            (baseResourceCost * (abMult - 1.0)) +
            (baseResourceCost * (apMult - 1.0)) +
            (baseResourceCost * (t5Mult - 1.0))
    }
    ...
    override fun cast(sp: SimParticipant) {
        ...
        val ccCrit =
            if (clearcasting != null) {
                arcanePotency?.addlClearcastingCritPct() ?: 0.0
            } else 0.0
