...
class Combustion : Buff() {
    ...
    val affectedSpells = listOf(Fireball.name, FireBlast.name, Scorch.name)

    fun consumeProc(buff: Buff): Proc =
        object : Proc() {
    ...
    fun stackProc(buff: Buff): Proc =
        object : Proc() {
            override val triggers: List<Trigger> = listOf(Trigger.SPELL_HIT)
            override val type: Type = Type.STATIC
    ...
    override fun procs(sp: SimParticipant): List<Proc> = listOf(consumeProc(this), stackProc(this))
}
