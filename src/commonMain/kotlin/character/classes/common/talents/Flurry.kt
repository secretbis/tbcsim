...
class Flurry(currentRank: Int) : Talent(currentRank) {
    ...
    val chargeProc =
        fun(buff: Buff): Proc {
            return object : Proc() {
                ...
                // Shaman Flurry stacks are not consumed on every hit - a small ICD is observed on
                // the
                // beta
    ...
    val hsProc =
        fun(buff: Buff): Proc {
            return object : Proc() {
                override val triggers: List<Trigger> = listOf(Trigger.MELEE_YELLOW_HIT)
                override val type: Type = Type.STATIC
    ...
    val hasteBuff =
        object : Buff() {
            ...
            override fun modifyStats(sp: SimParticipant): Stats {
                ...
                val state = state(sp)
                val modifier =
                    if (talentRanks > 0 && state.currentCharges > 0) {
    ...
    val onCritProc =
        object : Proc() {
            override val triggers: List<Trigger> =
                listOf(Trigger.MELEE_AUTO_CRIT, Trigger.MELEE_WHITE_CRIT, Trigger.MELEE_YELLOW_CRIT)
            override val type: Type = Type.STATIC
