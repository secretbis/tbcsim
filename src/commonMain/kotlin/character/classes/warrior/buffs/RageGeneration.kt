...
class RageGeneration : Buff() {
    ...
    fun damageToRage(sp: SimParticipant, damage: Double, item: Item, weaponFactor: Double): Int {
        ...
        return (((damage / rageConversionFactor * 7.5) + (item.speed / 1000.0 * weaponFactor)) / 2.0 * multiplier)
            .toInt()
    }
    ...
    val procHit =
        object : Proc() {
            override val triggers: List<Trigger> =
                listOf(Trigger.MELEE_WHITE_HIT, Trigger.MELEE_AUTO_HIT, Trigger.MELEE_BLOCK, Trigger.MELEE_GLANCE)
    ...
    val procCrit =
        object : Proc() {
            override val triggers: List<Trigger> = listOf(Trigger.MELEE_WHITE_CRIT, Trigger.MELEE_AUTO_CRIT)
            override val type: Type = Type.STATIC
