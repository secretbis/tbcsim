...
abstract class Gem(
    ...
    override var phase =
        if (_quality == Quality.EPIC) {
            3
        } else 1

    private val cleanPrefixName =
        if (prefix != null) {
            "${prefix.name.lowercase().replaceFirstChar { c -> c.uppercase() }} "
        } else ""
    ...
    open fun metaActive(sockets: List<Socket>): Boolean {
        ...
        return byColor[Color.RED] ?: 0 >= 2 && byColor[Color.YELLOW] ?: 0 >= 2 && byColor[Color.BLUE] ?: 0 >= 2
    }
}
