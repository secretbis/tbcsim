...
object SimStats {
    ...
    private fun dpsForParticipant(sp: SimParticipant): Double {
        return sp.events
            .filter { evt -> evt.eventType == EventType.DAMAGE }
            .fold(0.0) { acc, event -> acc + event.amount } / (sp.sim.opts.durationMs / 1000.0)
    }
    ...
    fun dps(iterations: List<SimIteration>): Map<String, DpsBreakdown?> {
        ...
        val perIteration =
            iterations.map {
                mapOf(
                    ...
                    "subjectPet" to
                        if (hasPet) {
                            dpsForParticipant(it.subject.pet!!)
                        } else null,
                )
            }
        ...
        val subjectPetData =
            if (hasPet) {
                perIteration.map { it["subjectPet"]!! }
            } else null

        val subjectMean = subjectData.average()
        val subjectPetMean = subjectPetData?.average()
        return mapOf(
            "subject" to DpsBreakdown(median(subjectData), mean = subjectMean, sd = sd(subjectData, subjectMean)),
            "subjectPet" to
                if (hasPet) {
                    DpsBreakdown(
    ...
    fun resultsByBuff(iterations: List<SimIteration>): List<List<BuffBreakdown>> {
        return processBuffs(iterations, EventType.BUFF_START, EventType.BUFF_REFRESH, EventType.BUFF_END)
    }
    ...
    private fun processBuffs(
        ...
        val participantCount =
            if (onlyTarget) {
                0
            } else (iterations[0].participants.size - 1)
        ...
        return (0..participantCount).map { idx ->
                ...
                iterations
                    .flatMap { iter ->
                        val context =
                            if (onlyTarget) {
                                iter.target
                            } else iter.participants[idx]
                        context.events
                            .filter {
                                it.buff != null && (!it.buff.hidden || showHidden) && eventTypes.contains(it.eventType)
                            }
            ...
            keys
                .map { key ->
                    ...
                    // Stack tracking - this is different because we need the size of each
                    // sub-segment
                    // start -> <subsegment 1> -> refresh -> <subsegment 2> -> refresh ->
                    // <subsegment 3> ->
                    // end
                    ...
                    events.forEach {
                        if (it.eventType == buffStart) {
                            if (lastEvent?.eventType == buffStart) {
                                logger.warn {
                                    "Possibly invalid segment - found two starts for buff: ${it.buff!!.name}"
                                }
                            }
                            ...
                            if (it.buffStacks != 0) {
                                if (lastStackSegmentEvent != null) {
                                    stackDurationsMs.add(
                                        Pair(it.timeMs - lastStackSegmentEvent.timeMs, lastStackSegmentEvent.buffStacks)
                                    )
                                    ...
                                    logger.warn {
                                        "Possibly invalid stack segment - refresh without prior start for buff: ${it.buff!!.name}"
                                    }
                                }
                            }
                        }
                        ...
                        if (it.eventType == buffRefresh) {
                            if (lastEvent?.eventType == buffEnd) {
                                logger.warn {
                                    "Possibly invalid segment - refresh without prior start for buff: ${it.buff!!.name}"
                                }
                            }
                            refreshCount++
                        }
                        ...
                        if (it.eventType == buffEnd) {
                            ...
                            if (currentStart != null) {
                                ...
                                logger.warn {
                                    "Possibly invalid segment - found end without start for buff: ${it.buff!!.name}"
                                }
                            }
                        }
                    ...
                    val uptimePct =
                        segments.sumOf { it.durationMs } /
                            (iterations.size * iterations[0].opts.durationMs).toDouble() * 100.0
    ...
    fun resultsByAbility(iterations: List<SimIteration>): List<List<AbilityBreakdown>> {
        ...
        return (0..participantCount).map { idx ->
            ...
            val grandTotal =
                keys.fold(0.0) { acc, it -> acc + (byAbility[it]?.sumOf { it.amount } ?: 0.0) } /
            ...
            keys
                .map { key ->
                    ...
                    val allHits =
                        events.filter {
                            it.result == EventResult.HIT ||
                                it.result == EventResult.BLOCK ||
                                it.result == EventResult.PARTIAL_RESIST_HIT
                        }
                    val allCrits =
                        events.filter {
                            it.result == EventResult.CRIT ||
                                it.result == EventResult.BLOCKED_CRIT ||
                                it.result == EventResult.PARTIAL_RESIST_CRIT
                        }
                    ...
                    val missPct =
                        events.filter { it.result == EventResult.MISS || it.result == EventResult.RESIST }.size /
                            amounts.size.toDouble() * 100.0
                    val dodgePct =
                        events.filter { it.result == EventResult.DODGE }.size / amounts.size.toDouble() * 100.0
                    val parryPct =
                        events.filter { it.result == EventResult.PARRY }.size / amounts.size.toDouble() * 100.0
                    val glancePct =
    ...
    fun resultsByDamageType(iterations: List<SimIteration>): List<List<DamageTypeBreakdown>> {
        ...
        return (0..participantCount).map { idx ->
            ...
            val grandTotal =
                keys.fold(0.0) { acc, it -> acc + (byDmgType[it]?.sumOf { it.amount } ?: 0.0) } /
                    iterations.size.toDouble()
            ...
            keys
                .map { key ->
                    ...
                    DamageTypeBreakdown(key.name, count, total, pctOfTotal)
                }
                .sortedBy { it.totalAvg }
                .reversed()
        }
    ...
    fun resourceUsage(iterations: List<SimIteration>): List<Map<String, ResourceBreakdown>> {
        ...
        return participants.mapIndexed { _, sp ->
            ...
            resourceTypes.fold(mutableMapOf()) { acc, resourceType ->
                val series =
                    sp.events
                        .filter { it.eventType == EventType.RESOURCE_CHANGED && it.resourceType == resourceType }
                        .map { Pair((it.timeMs / 1000.0).toInt(), it.amountPct) }

                acc[resourceType.name] = ResourceBreakdown(iterationIdx, series)
                acc
            }
        }
    }
