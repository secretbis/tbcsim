...
export default function () {
  ...
  function sim() {
    ...
    tbcsim.runSim(
      ...
      iterations => {
        ...
        dispatch({
          type: 'resultsResourceUsageByAbility',
          value: resourceUsageByAbility,
        });
  ...
  return (
    <Content>
      <Grid fluid={true}>
        <Message type='warning' header={bannerTitle} closable>
          {bannerMsg()}
        </Message>
        <Presets />
        <Row>
        ...
        <Row style={{ margin: '10px 0 0 0' }}>
          <Button appearance='ghost' disabled={simDisabled} onClick={onSimClick}>
            Sim!
          </Button>
          {state.iterationsCompleted == null && <CombatLog iterations={state.iterationResults} />}
          {state.iterationsCompleted != null && (
            <span style={{ marginLeft: '10px' }}>
              Iterations completed: {state.iterationsCompleted} of {state.simOptions.iterations}
            </span>
          )}
        </Row>
