...
const model = SchemaModel({
  durationSeconds: NumberType()
    .range(30, 600, 'Valid fight durations are 30-600s')
    .isRequired('A fight duration is required.'),
  durationVariabilitySeconds: NumberType()
    .range(0, 60, 'Valid fight variability amounts are 0-60s')
    .isRequired('A fight variability amount is required.'),
  stepMs: NumberType()
    .range(1, 1000, 'Valid step durations are 1-1000ms')
    .isRequired('A fight step size is required'),
  latencyMs: NumberType()
    .range(0, 500, 'Valid latencies are 0-500ms')
    .isRequired('A latency amount is required'),
  iterations: NumberType()
    .range(1, 10000, 'Valid iteration counts are 1-10000')
    .isRequired('Number of iterations is required'),
  targetLevel: NumberType()
    .range(70, 73, 'Valid target levels are 70-73')
    .isRequired('Target level is required'),
  targetType: NumberType().range(
    0,
    9,
    'Valid target types are 0-9, corresponding to internal constants',
  ),
  targetArmor: NumberType()
    .range(0, 10000, 'Valid target armor amounts are 0-10000')
    .isRequired('Target armor is required'),
...
export default function () {
  ...
  return (
    <Form checkTrigger={'change'} formValue={{ ...simOptions }} model={model} onChange={onChange}>
      <Row>
        <Col xs={12}>
          <Form.Stack layout={'horizontal'}>
            ...
            <Form.Group controlId='stepMs' style={groupStyle}>
              ...
              <Form.Text tooltip>
                How often to process the current simulation state. Set this lower for greater
                accuracy, and higher for faster sims
              </Form.Text>
            </Form.Group>
        ...
        <Col xs={12}>
          <Form.Stack layout={'horizontal'}>
            <Form.Group controlId='durationVariabilitySeconds' style={groupStyle}>
              ...
              <Form.Text tooltip>
                Randomly vary the length of each iteration by up to this amount, in seconds
              </Form.Text>
            </Form.Group>
            ...
            <Form.Group controlId='showHiddenBuffs' style={groupStyle}>
              ...
              <Form.Text tooltip>
                This is a debugging option to show extremely verbose buffs, that are normally
