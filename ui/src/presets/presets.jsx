...
const presets = {
  hunter: {
    preraid: [hunterBmPreraid, hunterSurvPreraid],
    phase1: [hunterBmPhase1, hunterSurvPhase1],
    phase2: [hunterBmPhase2, hunterSurvPhase2],
    phase3: [hunterBmPhase3, hunterSurvPhase3],
  },
  mage: {
    preraid: [mageArcanePreraid, mageFirePreraid, mageFrostPreraid],
    phase1: [mageArcanePhase1, mageFirePhase1, mageFrostPhase1],
    phase2: [mageArcanePhase2, mageFirePhase2, mageFrostPhase2],
    phase3: [mageArcanePhase3, mageFirePhase3, mageFrostPhase3],
  },
  priest: {
    preraid: [priestShadowPreraid],
    phase1: [priestShadowPhase1],
    phase2: [priestShadowPhase2],
    phase3: [priestShadowPhase3],
  },
  rogue: {
    preraid: [rogueAssassinationPreraid, rogueCombatPreraid],
    phase1: [rogueAssassinationPhase1, rogueCombatPhase1],
    phase2: [rogueAssassinationPhase2, rogueCombatPhase2],
    phase3: [rogueCombatPhase3, rogueCombatPhase3Glaives],
  },
  shaman: {
    preraid: [
      shamanElePreraid,
  ...
  warrior: {
    preraid: [warriorArmsPreraid, warriorFuryPreraid],
    phase1: [warriorArmsPhase1, warriorFuryPhase1],
    phase2: [warriorArmsPhase2, warriorFuryPhase2, warriorKebabPhase2, warriorProtectionPhase2],
    phase3: [
      warriorArmsPhase3,
      warriorFuryPhase3,
    ...
    <>
      <Dropdown title={title}>
        {racesForClass.map(race => {
          return (
            <Dropdown.Item key={race} eventKey={race} onSelect={onSelect}>
              {race}
            </Dropdown.Item>
          );
        })}
    ...
    <>
      <Dropdown title={title}>
        {targetTypes.map(type => {
          return (
            <Dropdown.Item key={type.key} eventKey={type.key} onSelect={onSelect}>
              {type.name}
            </Dropdown.Item>
          );
        })}
...
export default () => {
  ...
  function Export() {
    ...
    return (
      <Button disabled={!character.class} onClick={exportPreset}>
        Export
      </Button>
    );
  }
  ...
  function loadPreset(presetObj, dispatch) {
    ...
    clone.gear = _.mapValues(clone.gear, rawItem => {
      ...
      if (rawItem.tempEnchant) {
        const tmpEnchant = tbcsim.TempEnchants.getInstance()
          .byName.asJsMapView()
          .get(rawItem.tempEnchant);
        if (tmpEnchant) {
  ...
  function presetsFor(klass, category) {
    return (
      <>
        {(presets[klass][category] || [])
          .filter(it => it && it.epCategory === category)
          .map((p, idx) => {
