...
export function itemEp(item, category, spec, epOptions) {
  ...
  let itemEp = allStats.reduce((acc, current) => {
    ...
    if (currentStatEp) {
      ...
      const buffsEp =
        (item.buffs &&
          item.buffs &&
          item.buffs.asJsReadonlyArrayView().reduce((acc2, buff) => {
            const permanentStats = buff.permanentStats_18 && buff.permanentStats_18();
            const permanentStatsEp = permanentStats
              ? (permanentStats[current] || 0) * currentStatEp
              : 0;
  ...
  if (
    (options.benefitsFromMeleeWeaponDps && isMeleeWeapon(item)) ||
    (options.benefitsFromRangedWeaponDps && isRangedWeapon(item))
  ) {
