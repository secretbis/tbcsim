...
export default function ({ iterations }) {
  ...
  function renderLog() {
    const events = iterations
      ...
      [selectedIteration][selectedParticipant].events.asJsReadonlyArrayView();
    return (
      <Container>
  ...
  function iterationDps() {
    ...
    return <strong>{totalDps.total.toFixed(2)}</strong>;
  }
  ...
  return (
    <span style={{ marginLeft: 10 }}>
      <Button appearance='ghost' onClick={onCombatLogClick}>
        Combat Log
      </Button>
      <Modal
        open={modalOpen}
        ...
        <Modal.Header style={{ marginBottom: 10 }}>
          ...
          <div style={{ marginBottom: 10 }}>
            <InputNumber
              ...
              style={{ width: 170 }}
              min={0}
              max={iterations.size - 1}
              value={selectedIteration}
              onChange={onSelectedIterationChange}
            />
          </div>
